...
planck_map, mask_planck_2015 = LoadPlanck(fits_planck_map, fits_planck_mask, nside, do_plots=0, filt_plank_lmin=0)
...
est = cs.Master(mask, lmin=lmin, lmax=lmax, delta_ell=delta_ell, MASTER=1, pixwin=1)

clkg = np.zeros((nsims,est.lb.size))
clgg = np.zeros((nsims,est.lb.size))

for n in tqdm(xrange(nsims)):
	# map_gg = hpy.GetCorrMaps(clgg_th, nside, lmax=lmax_map, pixwin=1)
	map_gg_tot = hp.synfast(clgg_th + np.ones(lmax_map+1)*1./ngal_sr, nside, lmax_map, pixwin=1) #hpy.Counts2Delta(counts, mask=mask)
	clkg[n] = est.get_spectra(planck_map, map2=map_gg_tot, analytic_errors=0)
...
fname_kg = 'null_test_mock_gals_dl'+str(delta_ell)+'_lmin_'+str(lmin)+'_lmax'+str(lmax)+'_KS_min_'+str(K_S_min)+'_KS_max_'+str(K_S_max)+'_nside'+str(nside)+'_zmin_'+str(zmin)+'_zmax'+str(zmax)+'_nsims'+str(nsims)+'.pkl.gz'
